...
mydata = read.csv("EURUSD_remove_weekend.csv", header = TRUE)
...
write.csv(forecasts, file="forecasts_w1000.csv", row.names=FALSE)
...
spArimaGarch = as.xts( 
  read.zoo(
    file="forecasts_w1000_new.csv", format="%Y-%m-%d", header=F, sep=","
  )
)
